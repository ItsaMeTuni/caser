...
pub trait RowHelpers
{
    fn get_cell<'a, T: FromSql<'a>>(&'a self, col: &str) -> Result<T, DatabaseError>;
    fn get_cell_with_default<'a, T: FromSql<'a>>(&'a self, col: &str, default: T) -> Result<T, DatabaseError>;
}

impl RowHelpers for Row
{
    fn get_cell<'a, T: FromSql<'a>>(&'a self, col: &str) -> Result<T, DatabaseError>
    {
