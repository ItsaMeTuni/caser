...
use postgres::Row;
use crate::database_helpers::{FromRow, RowHelpers};
use chrono::{NaiveDate, NaiveDateTime, NaiveTime, Duration};
use caser_common::recurrence::RecurrenceRule;
use serde::{Serialize, Deserialize};
use uuid::Uuid;
use itertools::Itertools;
use crate::iter_helpers::MergeOrderedTrait;
use crate::recurrence::RecurrenceRuleInstance;
...
impl FromRow for Event
{
    ...
    fn from_row(row: &Row) -> Result<Self, DatabaseError>
    {
        if row.get_cell::<Option<String>>("rrule")?.is_some()
        {
...
impl EventRecurring
{
    ...
    pub fn generate_instances(&self, from_date: Option<NaiveDate>, to_date: Option<NaiveDate>, skip: usize, max_results: usize) -> Result<Vec<EventInstance>, DatabaseError>
    {
        ...
        let instances = RecurrenceRuleInstance::new(&self.recurrence.rule, self.span.get_date_span().start)
            .calculate_instances()
    ...
    fn from_row(row: &Row) -> Result<Self, DatabaseError>
    {
        ...
        let rrule_field: Option<&str> = row.get_cell("rrule")?;
        ...
        let recurrence = EventRecurrence {
            exdates: row.get_cell_with_default("exdates",  vec![])?,
            rdates: row.get_cell_with_default("rdates",  vec![])?,
        ...
        Ok(
            EventRecurring {
                ...
                span,
                recurrence,
                last_modified: row.get_cell("last_modified")?,
            }
        )
    }
}
...
impl EventSingle
{
    ...
    fn from_row(row: &Row) -> Result<Self, DatabaseError>
    {
        Ok(
            EventSingle {
                id: row.get_cell("id")?,
                parent_id: row.get_cell("parent_event_id")?,
...
impl EventInstance
{
    ...
    fn from_row(row: &Row) -> Result<Self, DatabaseError>
    {
        Ok(
            EventInstance {
                parent_id: row.get_cell("parent_id")?,
                span: EventSpan::from_row(row)?,
            }
        )
    }
